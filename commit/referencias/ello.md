...
**Pergunte ao usuário** se o commit sai ou não no changelog de deploy — mas só em projeto com changelog (verificação no passo 3.5 do `fluxo.md`; sem changelog, nem pergunte) — não assuma pela presença do ticket. A pergunta vale inclusive com ticket (o `(tt-XXXX)` normalmente implica "sai no changelog"; confirme, e se o usuário disser que não deve sair, aplique a exceção conforme a tabela acima).
