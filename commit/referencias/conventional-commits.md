...
4. Um escopo PODE ser fornecido após um tipo. Um escopo DEVE consistir em um substantivo descrevendo uma seção da base de código, cercado por parênteses, por exemplo `fix(parser):`
5. Uma descrição DEVE seguir imediatamente os dois-pontos e espaço após o prefixo tipo/escopo. A descrição é um resumo curto das alterações no código, por exemplo _fix: problema de parsing de array quando múltiplos espaços estavam contidos na string_.
...
9. O token de um rodapé DEVE usar `-` no lugar de espaços em branco, por exemplo `Acked-by` (isto ajuda a diferenciar a seção de rodapé de um corpo com vários parágrafos). Uma exceção é feita para `BREAKING CHANGE`, que TAMBÉM PODE ser usada como token.
...
12. Se incluída como rodapé, uma breaking change DEVE consistir no texto em maiúsculas BREAKING CHANGE, seguido por dois-pontos, espaço e descrição, por exemplo _BREAKING CHANGE: variáveis de ambiente agora têm precedência sobre arquivos de configuração_.
...
14. Tipos diferentes de `feat` e `fix` PODEM ser usados em suas mensagens de commit, por exemplo _docs: atualizar documentação de referência._
...
### Como devo versionar minhas extensões da Especificação Conventional Commits, por exemplo `@jameswomack/conventional-commit-spec`?
...
#### Quando você usou um tipo que está na especificação mas não é o tipo correto, por exemplo `fix` em vez de `feat`
...
#### Quando você usou um tipo _não_ da especificação, por exemplo `feet` em vez de `feat`
